...
class SeoService
{
        ...
        bool $shareCanonicalAcrossSites = null)
    {
        ...
        if (null === $shareCanonicalAcrossSites) {
            ...
            $shareCanonicalAcrossSites = (bool)variable_get('ucms_seo_share_canonical', true);
        }
     ...
     */
    public function setNodeMeta(NodeInterface $node, array $values = [])
    {
     ...
     * @return string
     */
    public function normalizeSegment(string $value, int $maxLength = self::SEGMENT_TRIM_LENGTH) : string
    {
        // Transliterate first
     ...
     *   to save a SQL query
     */
    public function setNodeSegment(NodeInterface $node, string $segment, string $previous = null)
    {
        if ($this->isNodeBlacklisted($node)) {
            return;
        }
